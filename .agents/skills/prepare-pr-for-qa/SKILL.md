...
description: Prepare an implementation branch for human QA by validating its Linear issue, diff, verification evidence, pull request, and tester-facing QA Brief. Invoke explicitly at the end of an implementation session, for example `/prepare-pr-for-qa FE-123`.
