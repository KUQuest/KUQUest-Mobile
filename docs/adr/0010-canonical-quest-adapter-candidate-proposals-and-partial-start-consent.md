...
For the mobile prototype, `src/features/questBoard/questFixtures.ts` (`questFixtures`) is the single fixture catalog and `src/features/questBoard/questFixtureAdapter.ts` (`questFixtureAdapter`) is the authoritative adapter and source of truth for Quest, Quest Team, Candidate Proposal/Quest Application, invitation, Assignment, Consent, Settlement, and Work Chat state. `src/features/questBoard/questWorkflow.ts` (`QuestWorkflow`) is the screen-facing facade: screens and routes consume its projections and actions, while it owns viewer-explicit projections, lifecycle time, subscriptions, and adapter delegation. Screens must not derive competing domain state or call the adapter directly; the exact lifecycle values are `QUEST_DRAFT`, `QUEST_OPEN`, `QUEST_ASSIGNED`, `QUEST_AWAITING_PARTIAL_GROUP_START_CONSENT`, `QUEST_AWAITING_EDIT_CONSENT`, `QUEST_IN_PROGRESS`, and `QUEST_CANCELLED`.
...
- The four deterministic prototype personas are Hirer, Applicant, Quest Team Leader, and Worker. `QuestWorkflow.reset()` delegates to the adapter and restores their Team, Proposal, invitation, Assignment, Consent, Settlement, Work Chat membership, and session-message state to the catalog seed.
- Work Chat IDs, membership, and capabilities come from the adapter through Quest Workflow. Prototype adapter messages are session-only and are not persisted to SecureStore or a local database.
...
- Use one fixture catalog, one canonical local adapter, and one screen-facing workflow facade. This keeps the four participation/mode combinations, exact consent states, persona scenarios, and reset behavior consistent while keeping the backend boundary explicit.
...
- SecureStore remains the only draft persistence boundary, while workflow reset provides deterministic scenario state without a second database.
