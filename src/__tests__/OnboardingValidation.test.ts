...
describe('onboarding validation', () => {
  test('accepts a complete first step', () => {
    expect(validateProfileBasics(createValidProfile(), false, messages)).toEqual({});
  });

  test('rejects telephone values that do not match the backend contract', () => {
    const errors = validateProfileBasics({ ...createValidProfile(), telephone: '812345678' }, false, messages);
  ...
  test('requires a student ID and terms acceptance for a new student', () => {
    const errors = validateProfileBasics({ ...createValidProfile(), studentId: '', acceptedTerms: false }, false, messages);
