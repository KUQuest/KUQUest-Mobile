...
describe("onboarding validation", () => {
  test("accepts a complete first step", () => {
    expect(
      validateProfileBasics(createValidProfile(), false, messages, true)
    ).toEqual({});
  });

  test("rejects telephone values that do not match the backend contract", () => {
    const errors = validateProfileBasics(
      { ...createValidProfile(), telephone: "812345678" },
      false,
      messages,
      true
    );
  ...
  test("requires a first and last name for the API payload", () => {
    const errors = validateProfileBasics(
      { ...createValidProfile(), name: "Student" },
      false,
      messages,
      true
    );
  ...
  test("requires a student ID and terms acceptance for a new student", () => {
    const errors = validateProfileBasics(
      { ...createValidProfile(), studentId: "", acceptedTerms: false },
      false,
      messages,
      true
    );
    expect(errors).toEqual({
      studentId: messages.invalidStudentId,
      acceptedTerms: messages.requiredField,
    });
  });

  test("rejects student IDs that do not match the ten-digit API contract", () => {
    const errors = validateProfileBasics(
      { ...createValidProfile(), studentId: "671234567" },
      false,
      messages,
      true
    );
  ...
  test("validates started certificate rows while allowing optional Portfolio Work images", () => {
    const errors = validateProfileDetails(
      {
        certificates: [
          { name: "", issuer: "KU", issuedAt: "2024/01/01", imageUri: "" },
        ],
  ...
  test("requires employment type for an experience row", () => {
    ...
    expect(errors).toEqual({
      experience_0_employmentType: messages.requiredField,
    });
  });
