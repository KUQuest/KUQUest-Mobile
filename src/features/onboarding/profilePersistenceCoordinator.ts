...
export type UnavailableProfileCollections = Partial<
  Record<"experience" | "portfolio" | "certificates", true>
>;
...
function isFilled(value: {
  ...
  return Boolean(
    value.name || value.issuer || value.issuedAt || value.imageUri
  );
}

function isWorkFilled(value: {
  title: string;
  detail: string;
  imageUri: string;
}): boolean {
...
function isExperienceFilled(value: {
  title: string;
...
export class ProfilePersistenceCoordinator {
  ...
  private readonly portfolioReplacements = new Map<
    string,
  ...
  async save(
    ...
    isEditMode: boolean,
    termsVersion?: string,
    options?: { unavailableCollections?: UnavailableProfileCollections }
  ): Promise<ProfilePersistenceResult> {
    this.draft = cloneDraft(form);
    this.completedSteps = [];

    if (!this.draft.occupation || !this.draft.department)
      throw new Error("Academic registration options are incomplete");
    if (!isEditMode && this.draft.acceptedTerms && !termsVersion)
      throw new Error("EXPO_PUBLIC_TERMS_VERSION is required");
    const academicRegistration = {
      ...
      termsVersion:
        !isEditMode && this.draft.acceptedTerms && termsVersion
          ? termsVersion
          : undefined,
    };
    await this.runStep("academic-registration", () =>
      api.updateAcademicRegistration(
    ...
    const profile = {
      ...
      ...(this.draft.description.trim()
        ? { bio: this.draft.description.trim() }
        : {}),
      telephone: this.draft.telephone,
      ...(this.draft.department ? { departmentId: this.draft.department } : {}),
    };
    await this.runStep("profile", () =>
      api.updateProfile(profile, this.mutationOptions("profile", profile))
    );

    const profileImage = this.draft.profileImage;
    if (
      isLocalAsset(profileImage) &&
      !this.uploadedAssets.has(`avatar:${profileImage}`)
    ) {
    ...
    if (!options?.unavailableCollections?.certificates)
      await this.saveCertificates(api);
    if (!options?.unavailableCollections?.portfolio)
      await this.savePortfolio(api);
    if (!options?.unavailableCollections?.experience)
  ...
  private async saveCertificates(api: StudentApi): Promise<void> {
    for (const [index, certificate] of this.draft.certificates.entries()) {
      ...
      if (id) {
        const existingId = id;
        await this.runStep(`certificate:${index}:update`, () =>
          api.updateCertificate(
            existingId,
            certificateData,
            this.mutationOptions(`certificate-${index}-update`, {
              id: existingId,
              certificateData,
            })
          )
        ...
        id = await this.runStepWithResult(`certificate:${index}:create`, () =>
      ...
      if (
        isLocalAsset(certificate.imageUri) &&
        !this.uploadedAssets.has(imageKey)
      ) {
        const asset = { uri: certificate.imageUri };
        await this.runStep(`certificate:${index}:image`, () =>
          api
            .uploadCertificateImage(
              certificateId,
              asset,
              this.mutationOptions(`certificate-${index}-image`, {
                certificateId,
                asset,
              })
            )
  ...
  private async savePortfolio(api: StudentApi): Promise<void> {
    for (const [index, work] of this.draft.works.entries()) {
      ...
      const replacement = isLocalAsset(currentWork.imageUri)
        ? this.portfolioReplacements.get(currentWork.imageUri)
        : undefined;
      if (replacement) {
        await this.runStep(`portfolio:${index}:remove-replaced`, () =>
          api.deletePortfolio(replacement.oldId)
        );
      ...
      const hasNewImage =
        isLocalAsset(currentWork.imageUri) &&
        !this.uploadedAssets.has(`portfolio:${currentWork.imageUri}`);
      if (currentWork.id && hasNewImage) {
        ...
        const replacementId = await this.runStepWithResult(
          `portfolio:${index}:replace-create`,
          () =>
            api.createPortfolio(
              portfolioData,
        ...
        this.portfolioReplacements.set(currentWork.imageUri, {
          oldId: currentWork.id,
          newId: replacementId,
        });
        this.updatePortfolioId(index, replacementId);
        await this.runStep(`portfolio:${index}:replace-delete`, () =>
          api.deletePortfolio(currentWork.id as string)
        );
        this.portfolioReplacements.delete(currentWork.imageUri);
        this.uploadedAssets.add(`portfolio:${currentWork.imageUri}`);
        ...
        const portfolioData = {
          title: currentWork.title.trim(),
          ...(currentWork.detail.trim()
            ? { description: currentWork.detail.trim() }
            : {}),
        };
        await this.runStep(`portfolio:${index}:update`, () =>
          api.updatePortfolio(
            currentWork.id as string,
            portfolioData,
            this.mutationOptions(`portfolio-${index}-update`, {
              id: currentWork.id,
              portfolioData,
            })
          )
        ...
        const id = await this.runStepWithResult(
          `portfolio:${index}:create`,
          () =>
            api.createPortfolio(
              portfolioData,
        ...
        if (isLocalAsset(currentWork.imageUri))
          this.uploadedAssets.add(`portfolio:${currentWork.imageUri}`);
      }
    }
  }
  ...
  private async saveExperiences(api: StudentApi): Promise<void> {
    for (const [index, experience] of this.draft.experiences.entries()) {
      ...
      if (experience.id) {
        await this.runStep(`experience:${index}:update`, () =>
          api
            .updateExperience(
              experience.id as string,
              experienceData,
              this.mutationOptions(`experience-${index}-update`, {
                id: experience.id,
                experienceData,
              })
            )
        ...
        const saved = await this.runStepWithResult(
          `experience:${index}:create`,
          () =>
  ...
  private async deletePendingRecords(
    api: StudentApi,
  ...
  private async runStepWithResult<T>(
    name: string,
    action: () => Promise<T>
  ): Promise<T> {
    try {
  ...
  private updateCertificateId(index: number, id: string): void {
    this.draft = {
      ...this.draft,
      certificates: this.draft.certificates.map((item, itemIndex) =>
  ...
  private updatePortfolioId(index: number, id: string): void {
    this.draft = {
      ...this.draft,
      works: this.draft.works.map((item, itemIndex) =>
        itemIndex === index ? { ...item, id } : item
      ),
    };
  }
  ...
  private updateExperienceId(index: number, id: string): void {
    this.draft = {
      ...this.draft,
      experiences: this.draft.experiences.map((item, itemIndex) =>
        itemIndex === index ? { ...item, id } : item
      ),
    };
  }
