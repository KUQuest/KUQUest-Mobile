  ...
  const content = (
    <>
      ...
      <View className={styles.rowContent}>
        ...
        {description ? <Text className={styles.rowDescription}>{description}</Text> : null}
      </View>
      {value ? <Text className={styles.rowValue}>{value}</Text> : null}
      {trailing ?? (onPress ? <ChevronRight color={colors.textMuted} size={20} strokeWidth={2} className={styles.chevron} /> : null)}
    </>
  );

  return onPress ? (
    <Pressable accessibilityRole="button" onPress={onPress} className={`${styles.row} ${last ? '' : styles.rowWithDivider}`} testID={testID}>
      {content}
    </Pressable>
  ) : (
    <View className={`${styles.row} ${last ? '' : styles.rowWithDivider}`} testID={testID}>
      {content}
    </View>
  );
}
  ...
  return (
    <SafeAreaView edges={['top', 'left', 'right']} className={styles.safeArea}>
      <View className="items-center flex-row h-[56px] px-[12px]">
        <Pressable accessibilityLabel={messages.back} accessibilityRole="button" className="items-center h-[48px] justify-center w-[48px]" onPress={() => router.back()} testID="settings-back">
      ...
      <View className={styles.content}>
        <View className={styles.section}>
          ...
          <View className={styles.sectionBody}>
            ...
            />
            <SettingsRow
              description={switchingAccount ? messages.switchingAccount : messages.switchAccountDescription}
              icon={UserRound}
              onPress={switchAccount}
              title={messages.switchAccount}
              testID="settings-switch-account"
            />
            {devOverlayEnabled ? <SettingsRow
              description={switchingAccount ? messages.switchingAccount : messages.devOverlayDescription}
              icon={Code2}
              onPress={switchAccount}
        ...
        <View className={styles.section}>
          ...
          <View className={styles.sectionBody}>
            <SettingsRow
              ...
              trailing={<View className={styles.switchHost}><Host matchContents seedColor={colors.primary}><Switch testID="settings-notifications" value={notificationsEnabled} onValueChange={setNotificationsEnabled} /></Host></View>}
            />
            <SettingsRow description={messages.languageDescription} icon={Globe2} title={messages.language} value={messages.systemLanguage} />
            <SettingsRow description={messages.appearanceDescription} icon={Moon} title={messages.appearance} value={messages.systemAppearance} last />
          </View>
        </View>
        ...
        <View className={styles.section}>
          ...
          <View className={styles.sectionBody}>
            <SettingsRow description={messages.helpDescription} icon={CircleHelp} onPress={() => undefined} title={messages.help} />
            <SettingsRow icon={FileText} onPress={() => undefined} title={messages.terms} />
            <SettingsRow icon={LockKeyhole} onPress={() => undefined} title={messages.privacy} last />
          </View>
        </View>
