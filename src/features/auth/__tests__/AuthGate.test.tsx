...
jest.mock("../AuthService", () => ({
  authService: {
    ...
    getRoutingDestination: (...args: unknown[]) =>
      mockGetRoutingDestination(...args),
  },
  isAuthNetworkError: (error: unknown) =>
    Boolean(
      error &&
      typeof error === "object" &&
      "message" in error &&
...
describe("AuthGate offline prototype fallback", () => {
  ...
  it("prefers an authenticated cold deep link over the default routing destination", async () => {
    ...
    mockGetInitialURL.mockImplementationOnce(
      () =>
  ...
  it("keeps the error and retry surface for an HTTP session failure", async () => {
    ...
    await waitFor(() =>
      expect(view.getByText("Unable to load your session")).toBeTruthy()
    );
