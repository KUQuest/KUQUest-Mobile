...
export class AuthService implements AuthAdapter {
  ...
  private readonly isSuccessResponse: (
  ...
  constructor(options: AuthServiceOptions = {}) {
    ...
    const nativeGoogleSigninModule =
      options.googleSignin === undefined ? loadNativeGoogleSignin() : null;
    ...
    this.isSuccessResponse =
      nativeGoogleSigninModule?.isSuccessResponse ??
      ((response): response is NativeGoogleSigninSuccessResponse =>
  ...
  async getRoutingDestination(): Promise<RoutingDestination> {
    try {
      ...
      if (error instanceof ApiError && error.status === 401) {
        authDebug("registration status rejected session", {
          status: error.status,
        });
        await this.authClient.signOut().catch(() => undefined);
        throw new AuthError("SESSION_EXPIRED");
      }
      authDebug("registration status failed", {
        message: getErrorMessage(error),
      });
      throw new AuthError(
        "API_ERROR",
        getErrorMessage(error) ?? "Unable to load registration status"
      );
    }
  }
  ...
  private async signInWithNativeGoogle(): Promise<AuthSession> {
    ...
    try {
      ...
      authDebug("native Google response received", {
        type: response.type,
        hasIdToken:
          response.type === "success" && Boolean(response.data.idToken),
      });
      ...
      authDebug("Google account received", {
      ...
      if (googleAccountChosen)
        await this.clearGoogleAccount(this.nativeGoogleSignin);
  ...
  private async clearGoogleAccount(
