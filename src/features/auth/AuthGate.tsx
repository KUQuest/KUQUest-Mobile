...
function DemoLaunchOverlay({
  onSelect,
}: {
  onSelect: (target: DemoLaunchTarget) => void;
}) {
  return (
    <View className="flex-1 justify-center bg-ku-background p-[24px]">
      <Text className="text-ku-primary font-ku-bold text-ku-title">
        Developer mode
      </Text>
      <Text className="text-ku-text-secondary text-ku-body mt-[8px] mb-[24px]">
        เลือกหน้าที่ต้องการเปิดสำหรับทดสอบ
      </Text>
      <Pressable
        accessibilityRole="button"
        className="bg-ku-primary rounded-ku-pill min-h-[52px] justify-center px-[20px] mb-[12px]"
        onPress={() => onSelect("login")}
        testID="dev-launch-login"
      >
        <Text className="text-ku-white font-ku-bold text-center">
          Login ปกติ
        </Text>
      </Pressable>
      <Pressable
        accessibilityRole="button"
        className="border border-ku-primary rounded-ku-pill min-h-[52px] justify-center px-[20px] mb-[12px]"
        onPress={() => onSelect("register")}
        testID="dev-launch-register"
      >
        <Text className="text-ku-primary font-ku-bold text-center">
          Register / Onboarding
        </Text>
      </Pressable>
      <Pressable
        accessibilityRole="button"
        className="border border-ku-border rounded-ku-pill min-h-[52px] justify-center px-[20px]"
        onPress={() => onSelect("home")}
        testID="dev-launch-home"
      >
        <Text className="text-ku-text-strong font-ku-bold text-center">
          เข้าแอปด้วย Demo data
        </Text>
      </Pressable>
    </View>
  );
...
export default function Index() {
  ...
  const [demoLaunchTarget, setDemoLaunchTarget] =
    useState<DemoLaunchTarget | null>(null);
  const [demoLaunchAvailable, setDemoLaunchAvailable] = useState(() =>
    authEnvironment.isDemoEnabled()
  );
  const [initialDemoRoute, setInitialDemoRoute] = useState<
    PrototypeScenarioRoute | undefined
  >();
  const [initialDemoRouteResolved, setInitialDemoRouteResolved] = useState(
    () => !__DEV__
  );
  ...
  const chooseDemoLaunchTarget = React.useCallback(
    (target: DemoLaunchTarget) => {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    const unsubscribe = authEnvironment.subscribeDeepLinks((route) => {
      if (!mounted) return;
      setInitialDemoRoute(route);
    });
    void authEnvironment.getInitialDeepLink().then((route) => {
      if (!mounted) return;
  ...
  useEffect(() => {
    ...
    if (demoLaunchAvailable || authEnvironment.isDemoEnabled()) {
      return () => {
        mounted = false;
      };
    }
    ...
    handleNavigate,
    initialDemoRoute,
    initialDemoRouteResolved,
    router,
  ]);

  const demoLaunchOverlayVisible =
    (demoLaunchAvailable || authEnvironment.isDemoEnabled()) &&
    demoLaunchTarget === null;
