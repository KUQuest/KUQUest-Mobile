...
export default function Profile() {
  ...
  if (loadError) {
    return <SafeAreaView style={styles.safeArea}><TopBar onBackPress={() => router.back()} /><Text style={styles.statusText}>{messages.error}</Text></SafeAreaView>;
  }
  if (!content) {
    return <SafeAreaView style={styles.safeArea}><TopBar onBackPress={() => router.back()} /><Text style={styles.statusText}>{messages.loading}</Text></SafeAreaView>;
  }

  return (
    <SafeAreaView style={styles.safeArea}>
      <TopBar onBackPress={() => router.back()} />
      <ScrollView contentContainerStyle={[styles.content, width >= 600 && styles.tabletContent]}>
