...
describe("Student Profile presentation", () => {
  it("gives an empty About section a clear Settings recovery action", async () => {
    ...
    const view = await render(
      <AboutMe
        ...
        emptyActionLabel="Manage in Settings"
  ...
  it("renders profile statistics and filters Reviews by star rating", async () => {
      ...
      <>
        <Reviews
          ...
          stats={{
            ...
            ratingAverage: 4.9,
            ratingCount: 3,
            distribution: { 5: 1, 4: 1, 3: 0, 2: 0, 1: 1 },
          }}
          sectionTitle="Reviews"
          emptyText="No reviews"
          allLabel="All"
          eligibleReviewsLabel={(count) => `${count} eligible Quest reviews`}
          filteredReviewsLabel={(count, rating) =>
            `Showing ${count} ${rating}-star reviews`
          }
    ...
    expect(
      view.getByRole("button", { name: "1 stars: 1 reviews" })
    ).toBeTruthy();
    ...
    expect(
      view.getByText("Showing 1 1-star reviews").props.accessibilityLiveRegion
    ).toBe("polite");
  });
  ...
  it("renders Certificate cards with issuer, year, and preview interaction", async () => {
    const view = await render(
      <Certificates
        ...
        previewUnavailableText="Image unavailable"
        closeLabel="Close preview"
        unavailableText="Certificate unavailable"
      />
    );
    ...
    fireEvent.press(
      view.getByRole("button", { name: "React Patterns preview" })
    );
    await waitFor(() =>
      expect(view.getAllByLabelText("Close preview").length).toBeGreaterThan(0)
    );
  });
  ...
  it("gives the certificate viewer image an explicit width", async () => {
    ...
    fireEvent.press(
      view.getByRole("button", { name: "Viewer layout preview" })
    );
    await waitFor(() =>
      expect(view.getByTestId("certificate-preview-image")).toBeTruthy()
    );

    expect(view.getByTestId("certificate-preview-image").props.style).toEqual(
      expect.objectContaining({ width: "100%" })
    );
    expect(view.getByTestId("certificate-preview-image").props.contentFit).toBe(
      "contain"
    );
  });
  ...
  it("shows a Certificate fallback and disables preview after image failure", async () => {
        ...
        certificates={[
          {
            ...
            issuer: "KU",
            issuedYear: "2024",
            link: "https://example.test/broken.png",
          },
        ]}
        sectionTitle="Certificates"
        emptyText="No certificates"
    ...
    fireEvent(view.getByLabelText("Broken certificate certificate"), "error", {
      nativeEvent: {},
    });
    ...
    expect(
      view.getByRole("button", { name: "Broken certificate Image unavailable" })
  ...
  it("disables a Certificate preview after the modal image fails", async () => {
    const view = await render(
      <Certificates
        certificates={[
          {
            id: "modal-broken-certificate",
            title: "Modal broken certificate",
            issuer: "KU",
            issuedYear: "2024",
            link: "https://example.test/modal-broken.png",
          },
        ]}
        sectionTitle="Certificates"
        emptyText="No certificates"
    ...
    fireEvent.press(
      view.getByRole("button", { name: "Modal broken certificate preview" })
    );
    await waitFor(() =>
      expect(view.getByLabelText("Close preview")).toBeTruthy()
    );
    fireEvent(view.getByTestId("certificate-preview-image"), "error", {
      nativeEvent: {},
    });

    await waitFor(() =>
      expect(view.getByText("Certificate unavailable")).toBeTruthy()
    );
    fireEvent.press(view.getByLabelText("Close preview"));
    expect(
      view.getByRole("button", {
        name: "Modal broken certificate Image unavailable",
      }).props.accessibilityState
    ).toMatchObject({ disabled: true });
  });
  ...
  it("does not show empty content when an Experience section is unavailable", async () => {
    const view = await render(
      <Experience
        ...
        locale="en"
        errorText="This section is temporarily unavailable."
  ...
  it("does not show an empty rating state when Profile Rating is unavailable", async () => {
    const view = await render(
      <Reviews
        ...
        ratingErrorText="This section is temporarily unavailable."
        ratingRetryLabel="Try again"
  ...
  it("uses the no-rating message when a Student has no Profile Rating", async () => {
    const view = await render(
      <ProfileStats
        ...
        questsLabel="Total Quests"
        noRatingLabel="No Profile Rating yet"
      />
    );
  ...
  it("keeps Portfolio Work usable when an optional image is missing", async () => {
    ...
    fireEvent.press(
      view.getByRole("button", { name: "Quest project: View project details" })
    );
    await waitFor(() =>
      expect(view.getByLabelText("Close project details")).toBeTruthy()
    );
  });
  ...
  it("shows every Portfolio Work image in the project details sheet", async () => {
        ...
        works={[
          {
            ...
            detail: "Two screenshots",
            imageUri: "https://example.test/cover.png",
            imageUris: [
    ...
    fireEvent.press(
      view.getByRole("button", {
        name: "Gallery project: View project details",
      })
    );
    await waitFor(() => {
