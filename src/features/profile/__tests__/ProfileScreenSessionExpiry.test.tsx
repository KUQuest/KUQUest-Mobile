...
jest.mock('../../../locales/LocaleProvider', () => ({ useLocale: () => ({ locale: 'en' }) }));
jest.mock('../loadProfileViewData', () => ({ loadProfileViewData: jest.fn() }));

const mockedLoad = loadProfileViewData as jest.MockedFunction<typeof loadProfileViewData>;
const renderScreen = async () => await render(
  <NavigationVisibilityProvider><ProfileScreen /></NavigationVisibilityProvider>,
);
...
describe('Student Profile screen — expired session', () => {
  ...
  it('redirects to the root route exactly once when the session is gone', async () => {
    mockedLoad.mockRejectedValue(new AuthError('SESSION_EXPIRED', 'No active session'));
  ...
  it('still surfaces the error state for non-auth failures', async () => {
    ...
    await waitFor(() => expect(view.getByText('Unable to load your profile.')).toBeTruthy());
