...
import { ArrowLeft, CalendarDays, ChevronRight, Image as ImageIcon, Pencil, Plus, Trash2 } from 'lucide-react-native';
...
import { formatDateForApi, validateBasics, validateCertificate, validateExperience, validatePortfolio } from './validation';
import { splitDisplayName, toBasicsForm, toCertificateForm, toExperienceForm, toPortfolioForm, type BasicsForm, type CertificateForm, type ExperienceForm, type PortfolioForm } from './types';
import type { CertificateEntry, ExperienceEntry, PortfolioEntry } from '../../api/contracts';
...
function getErrorText(error: unknown, messages: ProfileEditMessages): string {
  if (error instanceof AuthError || error instanceof ApiError && error.status === 401) return messages.sessionExpired;
  if (error instanceof ApiError && error.status === 409) return messages.conflict;
...
function ScreenHeader({ title, backLabel, onBack, action }: { title: string; backLabel: string; onBack: () => void; action?: React.ReactNode }) {
  return <View className={styles.header}>
    <Pressable accessibilityRole="button" accessibilityLabel={backLabel} className={styles.backButton} onPress={onBack}>
      <ArrowLeft color={colors.primaryDeep} size={24} strokeWidth={2.2} />
    </Pressable>
    <Text accessibilityRole="header" className={styles.headerTitle}>{title}</Text>
...
function ErrorState({ message, retry, retryLabel }: { message: string; retry: () => void; retryLabel: string }) {
  return <View className={styles.statusCard} accessibilityRole="alert">
    <Text className={styles.statusText}>{message}</Text>
    <Pressable accessibilityRole="button" className={styles.retryButton} onPress={retry}><Text className={styles.retryButtonText}>{retryLabel}</Text></Pressable>
  </View>;
}
...
function SaveBar({ label, disabled, onPress }: { label: string; disabled: boolean; onPress: () => void }) {
  const insets = useSafeAreaInsets();
  return <View className={styles.saveBar} style={{ paddingBottom: Math.max(insets.bottom, 24) }}><View className={styles.saveBarInner}><Button disabled={disabled} onPress={onPress}>{label}</Button></View></View>;
}

function useLeaveConfirmation(messages: ProfileEditMessages, dirty: boolean, onLeave: () => void) {
  return () => {
    if (!dirty) {
      onLeave();
      return;
...
function useUnsavedNavigationGuard(messages: ProfileEditMessages, dirty: boolean): () => void {
  ...
  React.useEffect(() => navigation.addListener('beforeRemove', (event) => {
    ...
    Alert.alert(messages.unsavedTitle, messages.unsavedMessage, [
      { text: messages.stay, style: 'cancel' },
      { text: messages.leave, style: 'destructive', onPress: () => {
...
function ImagePickerField({ label, uri, placeholder, removeLabel, onChange, onError }: { label: string; uri: string; placeholder: string; removeLabel: string; onChange: (uri: string) => void; onError: (message: string) => void }) {
...
function DateField({ label, value, placeholder, onChange, error, clearLabel, onClear }: { label: string; value: string; placeholder: string; onChange: (value: string) => void; error?: string; clearLabel?: string; onClear?: () => void }) {
...
function BasicsEditor({ data, onBack }: { data: ProfileEditData; onBack: () => void }) {
  ...
  const { locale } = useLocale();
  const messages = profileEditMessages[locale];
  const [form, setForm] = useState<BasicsForm>(() => toBasicsForm(data.profile));
  ...
  const setField = <K extends keyof BasicsForm>(field: K, value: BasicsForm[K]) => {
    setDirty(true);
    setForm((current) => ({ ...current, [field]: value }));
  };
  const save = async () => {
    const nextErrors = validateBasics(form.name, messages.required, messages.invalidName);
  ...
  return <SafeAreaView edges={['top', 'left', 'right']} className={styles.safeArea}>
    <KeyboardAvoidingView className={styles.screen} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      <SaveBar label={saving ? messages.saving : messages.save} disabled={saving} onPress={() => void save()} />
    </KeyboardAvoidingView>
  </SafeAreaView>;
}

function SectionListScreen({ section, data, onBack }: { section: Exclude<EditSection, 'basics'>; data: ProfileEditData; onBack: () => void }) {
  const router = useRouter();
  const { locale } = useLocale();
  const messages = profileEditMessages[locale];
  const title = section === 'experience' ? messages.experienceSection : section === 'portfolio' ? messages.portfolioSection : messages.certificatesSection;
  const items = section === 'experience' ? data.experiences : section === 'portfolio' ? data.portfolio : data.certificates;
  const openItem = (id?: string) => router.push(`/profile/edit/${section}?itemId=${encodeURIComponent(id ?? 'new')}`);
...
function ExperienceRow({ entry, presentLabel, onPress }: { entry: ExperienceEntry; presentLabel: string; onPress: () => void }) {
  return <Pressable accessibilityRole="button" className={styles.itemRow} onPress={onPress}><View className={styles.itemRowContent}><Text className={styles.itemTitle}>{entry.title}</Text><Text className={styles.itemMeta}>{entry.organization || entry.employmentType}</Text><Text className={styles.itemDescription}>{entry.endedAt ? `${entry.startedAt} – ${entry.endedAt}` : `${entry.startedAt} – ${presentLabel}`}</Text></View><Pencil color={colors.primary} size={18} /></Pressable>;
}

function PortfolioRow({ entry, noImageLabel, onPress }: { entry: PortfolioEntry; noImageLabel: string; onPress: () => void }) {
  const image = entry.images[0]?.url;
  const [imageFailed, setImageFailed] = useState(false);
  return <Pressable accessibilityRole="button" className={styles.itemRow} onPress={onPress}>{image && !imageFailed ? <Image source={{ uri: image }} onError={() => setImageFailed(true)} className={styles.itemImage} contentFit="cover" /> : <View className={styles.itemImageFallback}><Text className={styles.itemImageFallbackText}>{noImageLabel}</Text></View>}<View className={styles.itemRowContent}><Text className={styles.itemTitle}>{entry.title}</Text>{entry.description ? <Text className={styles.itemDescription} numberOfLines={2}>{entry.description}</Text> : null}</View><Pencil color={colors.primary} size={18} /></Pressable>;
}

function CertificateRow({ entry, noImageLabel, onPress }: { entry: CertificateEntry; noImageLabel: string; onPress: () => void }) {
  const [imageFailed, setImageFailed] = useState(false);
...
function ExperienceEditor({ entry, onBack }: { entry?: ExperienceEntry; onBack: () => void }) {
  const router = useRouter();
  const { locale } = useLocale();
  const messages = profileEditMessages[locale];
  const employmentTypes = onboardingMessages[locale].employmentTypes;
  const [form, setForm] = useState<ExperienceForm>(() => toExperienceForm(entry));
  const [dirty, setDirty] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [saveError, setSaveError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  ...
  const leave = useLeaveConfirmation(messages, dirty, onBack);
  const allowNavigation = useUnsavedNavigationGuard(messages, dirty);
  const setField = <K extends keyof ExperienceForm>(field: K, value: ExperienceForm[K]) => { setDirty(true); setForm((current) => ({ ...current, [field]: value })); };
  const save = async () => {
  ...
  const remove = () => {
    ...
    Alert.alert(messages.deleteTitle, messages.deleteMessage(entry.title), [
      ...
      { text: messages.confirmDelete, style: 'destructive', onPress: () => {
        setDeleting(true);
        void (async () => {
          try {
            ...
            if (!(await redirectIfSessionExpired(error, router))) setSaveError(getErrorText(error, messages));
          } finally {
            setDeleting(false);
          }
        })();
...
function PortfolioEditor({ entry, onBack }: { entry?: PortfolioEntry; onBack: () => void }) {
  ...
  const leave = useLeaveConfirmation(messages, dirty, onBack);
  const allowNavigation = useUnsavedNavigationGuard(messages, dirty);
  const setField = <K extends keyof PortfolioForm>(field: K, value: PortfolioForm[K]) => { setDirty(true); setForm((current) => ({ ...current, [field]: value })); };
  const save = async () => {
    ...
    try {
      ...
      if (entry?.id) {
        ...
        else if (!form.imageUri && entry.images[0]?.url) await api.deletePortfolioImage(entry.id);
      } else {
        await api.createPortfolio({ title: form.title.trim(), description: form.description.trim() || undefined, imageUris: isLocalAsset(form.imageUri) ? [form.imageUri] : [] });
      }
      allowNavigation();
      router.back();
    } catch (error) {
...
function CertificateEditor({ entry, onBack }: { entry?: CertificateEntry; onBack: () => void }) {
  const router = useRouter();
  const { locale } = useLocale();
  const messages = profileEditMessages[locale];
  const [form, setForm] = useState<CertificateForm>(() => toCertificateForm(entry));
  const [dirty, setDirty] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [saveError, setSaveError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  ...
  const leave = useLeaveConfirmation(messages, dirty, onBack);
  const allowNavigation = useUnsavedNavigationGuard(messages, dirty);
  const setField = <K extends keyof CertificateForm>(field: K, value: CertificateForm[K]) => { setDirty(true); setForm((current) => ({ ...current, [field]: value })); };
  const save = async () => {
    ...
    try {
      ...
      if (certificateId) {
        ...
        if (isLocalAsset(form.imageUri)) await api.uploadCertificateImage(certificateId, { uri: form.imageUri });
        else if (!form.imageUri && entry?.image) await api.deleteCertificateImage(certificateId);
      } else {
        certificateId = await api.createCertificate(payload);
...
export default function ProfileEditSectionScreen() {
  const router = useRouter();
  const { section: rawSection, itemId: rawItemId } = useLocalSearchParams<{ section?: string | string[]; itemId?: string | string[] }>();
  ...
  return <ProfileEditDataLoader loadingVariant={loadingVariant} onBack={onBack}>{(data) => {
    ...
    if (section === 'experience') {
      ...
      return itemId === 'new' ? <ExperienceEditor onBack={onBack} /> : itemId && entry ? <ExperienceEditor entry={entry} onBack={onBack} /> : <SectionListScreen section="experience" data={data} onBack={onBack} />;
    }
    if (section === 'portfolio') {
      const entry = data.portfolio.find((item) => item.id === itemId);
      return itemId === 'new' ? <PortfolioEditor onBack={onBack} /> : itemId && entry ? <PortfolioEditor entry={entry} onBack={onBack} /> : <SectionListScreen section="portfolio" data={data} onBack={onBack} />;
    }
    if (section === 'certificates') {
      const entry = data.certificates.find((item) => item.id === itemId);
      return itemId === 'new' ? <CertificateEditor onBack={onBack} /> : itemId && entry ? <CertificateEditor entry={entry} onBack={onBack} /> : <SectionListScreen section="certificates" data={data} onBack={onBack} />;
    }
    return <SectionListScreen section="experience" data={data} onBack={onBack} />;
