...
describe("Profile edit validation", () => {
  test("requires a full display name within backend field limits", () => {
    expect(validateBasics("Ada", "Required", "Full name")).toEqual({
      name: "Full name",
    });
    expect(validateBasics("Ada Lovelace", "Required", "Full name")).toEqual({});
    expect(
      validateBasics(`${"A".repeat(101)} Lovelace`, "Required", "Full name")
    ).toEqual({ name: "Full name" });
    expect(
      validateBasics(`Ada ${"L".repeat(101)}`, "Required", "Full name")
    ).toEqual({ name: "Full name" });
  });
  ...
  test("validates Portfolio Work and Certificates", () => {
    expect(
      validatePortfolio({ title: "", description: "", imageUri: "" }, messages)
    ).toEqual({ title: "Required" });
    expect(
      validateCertificate(
        { name: "AWS", issuer: "", issuedAt: "not-a-date", imageUri: "" },
        messages
      )
    ).toEqual({
      issuer: "Required",
