...
export function validateBasics(
  ...
  if (parts.length < 2 || parts[0].length > 100 || lastName.length > 100)
...
export function validateExperience(
  ...
  if (form.endedAt && !isDate(form.endedAt))
    errors.endedAt = messages.invalidDate;
  if (
    form.endedAt &&
    isDate(form.startedAt) &&
    isDate(form.endedAt) &&
    form.endedAt < form.startedAt
  )
    errors.endedAt = messages.invalidExperienceDates;
  return errors;
}
...
export function validateCertificate(
  ...
  messages: { required: string; invalidDate: string }
): FormErrors {
  const errors: FormErrors = {};
