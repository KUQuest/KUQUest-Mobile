...
describe("ChatConversationScreen", () => {
  beforeEach(() => {
    ...
    questFixtureAdapter.joinDirect(
      "print-documents",
      "student-demo",
  ...
  it("opens a dynamic Quest owner conversation and sends a message", async () => {
    ...
    expect(
      view.getAllByText("Photocopy course documents").length
    ).toBeGreaterThan(0);
    expect(view.getByText("Mild P. · Quest owner")).toBeTruthy();
    expect(
      view.getByText("Hi! Feel free to ask about this Quest.")
    ).toBeTruthy();

    await fireEvent.changeText(
      view.getByLabelText("Message the Quest owner…"),
      "Could you confirm the meeting point?"
    );
  ...
  it("opens report review from a joined Quest chat with its context", async () => {
    const view = await render(<ChatConversationScreen />);

    expect(view.getByTestId("chat-report-button")).toBeTruthy();
    await fireEvent.press(view.getByTestId("chat-report-button"));
    ...
    expect(mockRouter.push).toHaveBeenCalledWith({
      ...
      params: {
        ...
        reportedMemberId: "student-creator-3",
      },
    });
  });
