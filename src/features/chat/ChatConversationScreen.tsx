      ...
      <View className={cn(styles.messageStack, mine && styles.messageStackMe)}>
        ...
        {message.attachment ? (
          <AttachmentRow
            ...
            messages={messages}
            mine={mine}
            onPress={() => onFilePress(message.attachment as ChatAttachment)}
          />
        ) : null}
...
export default function ChatConversationScreen() {
  ...
  const files = useMemo(
    () =>
      displayedMessages
        .flatMap((message) =>
          message.attachment
            ? [{ ...message.attachment, time: message.time }]
            : []
        )
        .filter(
          (file) =>
  ...
  const conversationPending =
    Boolean(conversationId && viewerId) &&
    loadStateForRoute.status === "pending";
  const conversationLoadFailed =
    Boolean(conversationId && viewerId) && loadStateForRoute.status === "error";
  ...
  const openAttachmentMenu = () => {
    ...
    return Alert.alert(
      ...
      messages.mockAttachmentDescription,
      [
