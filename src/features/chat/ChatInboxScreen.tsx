...
function ChatInboxSkeleton({ loadingLabel }: { loadingLabel: string }) {
  return (
    <LoadingSkeleton loadingLabel={loadingLabel} style={{ width: '100%' }} contentStyle={{ gap: spacing.sm }} testID="chat-inbox-loading-skeleton">
      {[1, 2, 3, 4].map((item) => <View key={item} className={styles.conversationRow} testID={`chat-skeleton-${item}`}>
        ...
        <View style={{ flex: 1, gap: spacing.xs, marginLeft: spacing.md }}>
          ...
          <SkeletonBlock height={14} width="88%" borderRadius={4} style={{ marginTop: spacing.xs }} />
        </View>
...
export default function ChatInboxScreen({ viewerId }: ChatInboxScreenProps) {
  ...
  const resolvedViewerId = viewerId?.trim() || activePersonaId || DEFAULT_PROTOTYPE_VIEWER_ID;
  ...
  const loadStateForViewer = loadState.viewerId === resolvedViewerId
    ? loadState
    : { viewerId: resolvedViewerId, status: 'pending' as const, conversations: [] };
  ...
  const conversations = useMemo(() => loadStateForViewer.conversations.filter((conversation) => {
        ...
        <View className={styles.content}>
          <View className={styles.listContent}>
            ...
            <View className={styles.sectionHeading}>
              ...
              {conversationsPending ? <SkeletonBlock height={16} width={78} borderRadius={4} testID="chat-inbox-loading-count" /> : <Text className={styles.sectionCount}>{messages.conversationCount(conversations.length)}</Text>}
            </View>
            {conversationsLoadFailed ? (
              <View accessibilityRole="alert" accessibilityLiveRegion="assertive" className={styles.loadErrorState}>
                ...
                <Pressable accessibilityRole="button" className={styles.loadErrorAction} onPress={() => {
                  setLoadState({ viewerId: resolvedViewerId, status: 'pending', conversations: [] });
                  setLoadAttempt((attempt) => attempt + 1);
                }}>
                  <Text className={styles.loadErrorActionText}>{messages.retry}</Text>
                </Pressable>
              </View>
            ) : conversationsPending ? <ChatInboxSkeleton loadingLabel={messages.loading} /> : conversations.length > 0 ? (
