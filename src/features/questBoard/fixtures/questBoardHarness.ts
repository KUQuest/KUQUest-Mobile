...
} from "../workflow/questWorkflowTypes";
...
export function parseBoardPreviewState(
  value: string | string[] | undefined
): BoardPreviewState | undefined {
  if (Array.isArray(value) && value.length !== 1) return undefined;
  const candidate = Array.isArray(value) ? value[0] : value;
  return previewOptions.find((option) => option.value === candidate)?.value;
}

export function getQuestDetailFixture(
  routeId: string | undefined,
  previewState?: BoardPreviewState
): QuestBoardQuest | undefined {
  ...
  if (previewState === "full" || previewState === "application-accepted")
    return { ...quest, acceptedParticipants: quest.headcount };
