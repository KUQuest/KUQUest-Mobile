...
describe("QuestWorkflow", () => {
  ...
  it("exposes board and detail projections from the canonical adapter", () => {
    ...
    expect(
      questWorkflow.getQuestBoardQuest(
        firstQuest.id,
        DEFAULT_PROTOTYPE_VIEWER_ID
      )
    ).toEqual(firstQuest);
  ...
  it("owns board surface states and viewer-specific My Quests projections", () => {
    ...
    ).toMatchObject({ kind: "unavailable", availability: "full" });

    const projections = questWorkflow.getMyQuestsProjection(
      DEFAULT_PROTOTYPE_VIEWER_ID
    );
    expect(
      projections.find(
        (projection) => projection.quest.id === "worker-pending-demo"
      )
    ).toMatchObject({
    ...
    expect(
      projections.find((projection) => projection.quest.id === "buy-lunch")
    ).toMatchObject({
      relationship: "worker",
      tab: "accepted",
      hasAssignment: true,
      groupChatCapability: expect.objectContaining({
        canRead: true,
        canWrite: true,
      }),
    });
  ...
  it("derives detail and settlement projections for the explicit viewer", () => {
    ...
    const adapter = createQuestFixtureAdapter({
    ...
    workflow.dispatch({
      type: "COMPLETE",
      questId: "buy-lunch",
      hirerId: "student-creator-4",
    });

    expect(
      workflow.getSettlement("buy-lunch", DEFAULT_PROTOTYPE_VIEWER_ID)
    ).toMatchObject({ requestedHeadcount: 1, actualHeadcount: 1 });
  });
  ...
  it("routes domain actions and preserves subscription refresh", () => {
    ...
    expect(
      workflow.getQuestDetailState(
        "print-documents",
        DEFAULT_PROTOTYPE_VIEWER_ID
      )?.assignments
    ).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ workerId: DEFAULT_PROTOTYPE_VIEWER_ID }),
      ])
    );
