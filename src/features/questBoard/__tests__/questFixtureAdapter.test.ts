...
describe("Quest fixture adapter", () => {
  ...
  it("keeps raw lifecycle values and performs a direct solo join in the adapter", () => {
    const before = questFixtureAdapter.getState(
      "print-documents",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      fixedNow
    );
    ...
    const result = questFixtureAdapter.joinDirect(
      "print-documents",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      fixedNow
    );
    ...
    if (result.ok) {
      ...
      expect(result.state.conversation.conversationId).toBe(
  ...
  it("creates and publishes a valid Quest into the Hirer state and removes it on reset", () => {
    const result = questFixtureAdapter.createAndPublishQuest(
      createPayload,
      "demo-hirer",
      fixedNow
    );
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.state.quest).toEqual(
    ...
    expect(result.state.quest.reward.rewardSatang).toBe(
      createPayload.rewardSatang
    );
    expect(result.state.settlement).toBeUndefined();
    expect(
      questFixtureAdapter.getPublishCheck(result.state.quest.id, fixedNow)
        ?.escrow
    ).toEqual(
      expect.objectContaining({
        headcount: 3,
    ...
    expect(
      questFixtureAdapter.getState(
        result.state.quest.id,
        "demo-hirer",
        fixedNow
      )?.quest.status
    ).toBe(QuestStatus.QUEST_OPEN);
    expect(
      questFixtureAdapter
        .listStates("demo-hirer", fixedNow)
        .some((state) => state.quest.id === result.state.quest.id)
    ).toBe(true);
  ...
  it("keeps create and publish headcount invariants and blocks invalid payloads", () => {
    const validSingle = questFixtureAdapter.createAndPublishQuest(
      { ...createPayload, participation: "SOLO", headcount: 1 },
      "demo-hirer",
      fixedNow
    );
    ...
    questFixtureAdapter.reset();
    const invalidSingle = questFixtureAdapter.createAndPublishQuest(
      { ...createPayload, participation: "SOLO", headcount: 2 },
      "demo-hirer",
      fixedNow
    );
    expect(invalidSingle.ok).toBe(false);
    if (!invalidSingle.ok)
      expect(invalidSingle.error).toEqual(
        expect.objectContaining({ code: "PUBLISH_BLOCKED" })
      );

    const invalidGroup = questFixtureAdapter.createAndPublishQuest(
      { ...createPayload, headcount: 0 },
      "demo-hirer",
      fixedNow
    );
    expect(invalidGroup.ok).toBe(false);
    if (!invalidGroup.ok)
      expect(invalidGroup.error).toEqual(
        expect.objectContaining({ code: "PUBLISH_BLOCKED" })
      );
    ...
    expect(
      questFixtureAdapter
        .listStates("demo-hirer", fixedNow)
        .some((state) => state.quest.title === createPayload.title)
    ).toBe(false);
  });

  it("creates and publishes through the typed adapter action without storing a draft", () => {
    const created = questFixtureAdapter.createQuest(
      createPayload,
      "demo-hirer",
      fixedNow
    );
    ...
    questFixtureAdapter.reset();
    const dispatched = questFixtureAdapter.dispatch(
      {
    ...
    if (dispatched.ok)
      expect(dispatched.state.quest.status).toBe(QuestStatus.QUEST_OPEN);
  });
  ...
  it("projects an assigned Quest into progress at start time without mutating the fixture", () => {
    ...
    const beforeStart = created.getState(
      "buy-lunch",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      new Date("2026-08-13T09:59:59.999Z")
    );
    ...
    expect(atStart?.capabilities.availableActions).toContain(
      "CONFIRM_COMPLETION"
    );

    const stored = created.getState(
      "buy-lunch",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      fixedNow
    );
  ...
  it("enters the partial-start demo directly with a frozen pending roster and voter capabilities", () => {
    const created = createQuestFixtureAdapter({ now: fixedNow });
    const demoWorker = created.getState(
      "partial-group-start-demo",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      fixedNow
    );
    const demoWorkerB = created.getState(
      "partial-group-start-demo",
      "demo-worker-2",
      fixedNow
    );
    const demoHirer = created.getState(
      "partial-group-start-demo",
      "demo-hirer",
      fixedNow
    );
    const nonVoter = created.getState(
      "partial-group-start-demo",
      "demo-worker-3",
      fixedNow
    );

    expect(demoWorker?.quest.status).toBe(
    ...
    expect(
      getConsentRemainingMs(demoWorker?.partialStartConsent, fixedNow)
    ).toBe(PARTIAL_GROUP_START_CONSENT_WINDOW_MS);
    expect(demoWorker?.capabilities.availableActions).toContain(
      "VOTE_PARTIAL_GROUP_START_CONSENT"
    );
    expect(demoWorker?.capabilities.availableActions).not.toContain(
      "DIRECT_JOIN"
    );
    expect(demoWorker?.conversation).toMatchObject({
      canRead: true,
      canWrite: true,
      readOnly: false,
    });
    expect(demoWorkerB?.capabilities.availableActions).toContain(
      "VOTE_PARTIAL_GROUP_START_CONSENT"
    );
    expect(demoHirer?.capabilities.availableActions).toContain(
      "VOTE_PARTIAL_GROUP_START_CONSENT"
    );
    expect(demoHirer?.capabilities.availableActions).not.toContain(
      "DIRECT_JOIN"
    );
    expect(nonVoter?.capabilities.availableActions).not.toContain(
      "VOTE_PARTIAL_GROUP_START_CONSENT"
    );

    const forbiddenVote = created.votePartialGroupStartConsent(
      "partial-group-start-demo",
      "demo-worker-3",
      false,
      fixedNow
    ...
    const demoLateJoin = created.joinDirect(
      "partial-group-start-demo",
      "demo-worker-3",
      fixedNow
    );
    ...
    if (!demoLateJoin.ok)
      expect(demoLateJoin.error.code).toBe("INVALID_STATUS");
    ...
    expect(atStart?.quest.status).toBe(
      QuestStatus.QUEST_AWAITING_PARTIAL_GROUP_START_CONSENT
    );
    expect(atStart?.actualHeadcount).toBe(1);
    expect(atStart?.partialStartConsent?.status).toBe(
      QuestPartialStartConsentStatus.PARTIAL_START_PENDING
    );
    expect(atStart?.partialStartConsent?.requiredVoterIds).toEqual([
      "student-creator-2",
      "demo-worker-3",
    ]);
    expect(atStart?.partialStartConsent?.frozenWorkerIds).toEqual([
      "demo-worker-3",
    ]);
    expect(atStart?.partialStartConsent?.responseDeadlineAt).toBe(
      "2026-08-13T09:05:00.000Z"
    );
    expect(
      getConsentRemainingMs(atStart?.partialStartConsent, directGroupStart)
    ).toBe(PARTIAL_GROUP_START_CONSENT_WINDOW_MS);
    ...
    expect(
      created
        .listBoardQuests("demo-worker-3", directGroupStart)
        .some((quest) => quest.id === "clean-fan")
    ).toBe(false);

    const lateJoin = created.joinDirect(
      "clean-fan",
      "demo-worker-4",
      new Date(directGroupStart.getTime() + 60_000)
    );
    expect(lateJoin.ok).toBe(false);
    if (!lateJoin.ok) {
      expect(lateJoin.error.code).toBe("INVALID_STATUS");
      expect(lateJoin.state.quest.status).toBe(
        QuestStatus.QUEST_AWAITING_PARTIAL_GROUP_START_CONSENT
      );
      expect(lateJoin.state.assignments).toHaveLength(1);
      expect(lateJoin.state.partialStartConsent?.frozenWorkerIds).toEqual([
        "demo-worker-3",
      ]);
    }
  });
  ...
  it("starts the seeded partial Group after the Hirer and both joined Workers approve", () => {
    ...
    const firstWorkerVote = created.votePartialGroupStartConsent(
      "partial-group-start-demo",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      true,
      fixedNow
    ...
    expect(firstWorkerVote.state.quest.status).toBe(
      QuestStatus.QUEST_AWAITING_PARTIAL_GROUP_START_CONSENT
    );
    expect(firstWorkerVote.state.partialStartConsent?.approvedVoterCount).toBe(
      1
    );
    expect(firstWorkerVote.state.capabilities.availableActions).not.toContain(
      "VOTE_PARTIAL_GROUP_START_CONSENT"
    );
    expect(
      created.getState("partial-group-start-demo", "demo-worker-2", fixedNow)
        ?.capabilities.availableActions
    ).toContain("VOTE_PARTIAL_GROUP_START_CONSENT");

    const hirerVote = created.votePartialGroupStartConsent(
      "partial-group-start-demo",
      "demo-hirer",
      true,
      fixedNow
    ...
    expect(hirerVote.state.quest.status).toBe(
      QuestStatus.QUEST_AWAITING_PARTIAL_GROUP_START_CONSENT
    );
    expect(hirerVote.state.partialStartConsent?.approvedVoterCount).toBe(2);

    const finalWorkerVote = created.votePartialGroupStartConsent(
      "partial-group-start-demo",
      "demo-worker-2",
      true,
      fixedNow
    ...
    if (finalWorkerVote.ok) {
      expect(finalWorkerVote.state.quest.status).toBe(
        QuestStatus.QUEST_IN_PROGRESS
      );
      expect(finalWorkerVote.state.partialStartConsent?.status).toBe(
        QuestPartialStartConsentStatus.PARTIAL_START_APPROVED
      );
      expect(
        finalWorkerVote.state.partialStartConsent?.approvedVoterCount
      ).toBe(3);
      expect(finalWorkerVote.state.actualHeadcount).toBe(2);
      expect(finalWorkerVote.state.assignments).toHaveLength(2);
      expect(
        finalWorkerVote.state.assignments.every(
          (item) => item.startedAt === finalWorkerVote.state.quest.startAt
        )
      ).toBe(true);
      ...
      expect(finalWorkerVote.state.capabilities.availableActions).toContain(
        "CONFIRM_COMPLETION"
      );
      expect(finalWorkerVote.state.capabilities.availableActions).not.toContain(
        "VOTE_PARTIAL_GROUP_START_CONSENT"
      );
      expect(finalWorkerVote.state.conversation).toMatchObject({
        canRead: true,
        canWrite: true,
        readOnly: false,
      });
      expect(
        created.getState("partial-group-start-demo", "demo-hirer", fixedNow)
          ?.quest.status
      ).toBe(QuestStatus.QUEST_IN_PROGRESS);
      expect(
        created.getConversationCapability(
          "partial-group-start-demo",
          "demo-worker-2",
          fixedNow
        )
      ).toMatchObject({ canRead: true, canWrite: true, readOnly: false });
    }
  });
  ...
  it("starts a partial direct Group after unanimous Hirer and Worker approval", () => {
    ...
    expect(hirerVote.state.partialStartConsent?.responses[0]?.role).toBe(
      "HIRER"
    );

    const workerVote = created.votePartialGroupStartConsent(
      "clean-fan",
      "demo-worker-3",
      true,
      new Date(directGroupStart.getTime() + 4 * 60_000)
    );
    expect(workerVote.ok).toBe(true);
    if (workerVote.ok) {
      expect(workerVote.state.quest.status).toBe(QuestStatus.QUEST_IN_PROGRESS);
      expect(workerVote.state.partialStartConsent?.status).toBe(
        QuestPartialStartConsentStatus.PARTIAL_START_APPROVED
      );
      ...
      expect(workerVote.state.assignments[0]?.startedAt).toBe(
  ...
  it("cancels a partial direct Group immediately on rejection with a full refund", () => {
    ...
    const rejected = created.votePartialGroupStartConsent(
      "clean-fan",
      "demo-worker-3",
      false,
      directGroupStart
    ...
    if (rejected.ok) {
      ...
      expect(rejected.state.partialStartConsent?.status).toBe(
  ...
  it("times out partial direct Group consent after exactly five minutes", () => {
    ...
    const joined = created.joinDirect("clean-fan", "demo-worker-3", fixedNow);
    expect(joined.ok).toBe(true);
    const opened = created.votePartialGroupStartConsent(
      "clean-fan",
      "student-creator-2",
      true,
      directGroupStart
    ...
    const justBeforeTimeout = created.getState(
      "clean-fan",
      "demo-worker-3",
      new Date(
        directGroupStart.getTime() + PARTIAL_GROUP_START_CONSENT_WINDOW_MS - 1
      )
    );
    expect(justBeforeTimeout?.quest.status).toBe(
      QuestStatus.QUEST_AWAITING_PARTIAL_GROUP_START_CONSENT
    );
    expect(justBeforeTimeout?.partialStartConsent?.status).toBe(
      QuestPartialStartConsentStatus.PARTIAL_START_PENDING
    );
    const oneMillisecondRemaining = new Date(
      directGroupStart.getTime() + PARTIAL_GROUP_START_CONSENT_WINDOW_MS - 1
    );
    expect(
      getConsentRemainingMs(
        justBeforeTimeout?.partialStartConsent,
        oneMillisecondRemaining
      )
    ).toBe(1);
    expect(
      formatConsentCountdown(
    ...
    const atTimeout = created.getState(
      "clean-fan",
      "demo-worker-3",
      new Date(
        directGroupStart.getTime() + PARTIAL_GROUP_START_CONSENT_WINDOW_MS
      )
    );
    expect(atTimeout?.quest.status).toBe(QuestStatus.QUEST_CANCELLED);
    expect(atTimeout?.partialStartConsent?.status).toBe(
      QuestPartialStartConsentStatus.PARTIAL_START_TIMED_OUT
    );
    expect(atTimeout?.assignments).toHaveLength(0);
    expect(atTimeout?.settlement).toMatchObject({
      actualHeadcount: 0,
      refundSatang: 30000,
      fullRefund: true,
    });
    ...
    expect(stored?.quest.status).toBe(
      QuestStatus.QUEST_AWAITING_PARTIAL_GROUP_START_CONSENT
    );
    expect(stored?.assignments).toHaveLength(1);
    expect(stored?.partialStartConsent?.status).toBe(
      QuestPartialStartConsentStatus.PARTIAL_START_PENDING
    );
  });
  ...
  it("projects a renamed edit-consent timeout back to its prior status without mutating it", () => {
    const created = createQuestFixtureAdapter({ now: fixedNow });
    const beforeTimeout = created.getState(
      "walk-together",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      fixedNow
    );
    const justBeforeTimeout = created.getState(
      "walk-together",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      new Date("2026-08-12T09:04:59.999Z")
    );
    const atTimeout = created.getState(
      "walk-together",
      DEFAULT_PROTOTYPE_VIEWER_ID,
    ...
    expect(beforeTimeout?.quest.status).toBe(
      QuestStatus.QUEST_AWAITING_EDIT_CONSENT
    );
    expect(justBeforeTimeout?.quest.status).toBe(
      QuestStatus.QUEST_AWAITING_EDIT_CONSENT
    );
    expect(justBeforeTimeout?.editConsent?.status).toBe(
      QuestEditRequestStatus.EDIT_REQUEST_PENDING
    );
    expect(beforeTimeout?.editConsent?.status).toBe(
      QuestEditRequestStatus.EDIT_REQUEST_PENDING
    );
    expect(beforeTimeout?.capabilities.availableActions).toContain(
      "VOTE_EDIT_CONSENT"
    );
    expect(atTimeout?.quest.status).toBe(QuestStatus.QUEST_IN_PROGRESS);
    expect(atTimeout?.editConsent?.status).toBe(
      QuestEditRequestStatus.EDIT_REQUEST_REJECTED
    );
    expect(atTimeout?.quest.location.label).toBe("สวนวชิรเบญจทัศ");
    expect(atTimeout?.capabilities.availableActions).not.toContain(
      "VOTE_EDIT_CONSENT"
    );

    const timedOutVote = created.voteEditConsent(
      "walk-together",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      true,
      new Date("2026-08-12T09:05:00.000Z")
    ...
    if (!timedOutVote.ok) {
      ...
      expect(timedOutVote.state.quest.status).toBe(
        QuestStatus.QUEST_IN_PROGRESS
      );
    }
    ...
    expect(stored?.editConsent?.status).toBe(
      QuestEditRequestStatus.EDIT_REQUEST_PENDING
    );
  });
  ...
  it("creates a pending Candidate application without creating Work Chat access", () => {
    const result = questFixtureAdapter.applyCandidate(
      "move-boxes",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      fixedNow
    );
    ...
    if (result.ok) {
      expect(
        result.state.applications.some(
          (item) =>
            item.applicantId === DEFAULT_PROTOTYPE_VIEWER_ID &&
  ...
  it("manually rejects one individual Candidate Proposal without selecting or creating chat", () => {
    const target = questFixtureAdapter
      .getState("single-candidate-demo", "demo-hirer", fixedNow)
      ?.applications.find((item) => item.applicantId === "single-applicant-b");
    ...
    if (rejected.ok) {
      ...
      expect(
        rejected.state.applications.find((item) => item.id === target.id)
      ).toEqual(
        expect.objectContaining({
      ...
      expect(
        rejected.state.applications.filter(
          (item) => item.status === QuestApplicationStatus.APPLICATION_APPLIED
        )
      ).toHaveLength(2);
      expect(rejected.state.assignments).toHaveLength(0);
    ...
    const resubmitted = questFixtureAdapter.applyCandidate(
      "single-candidate-demo",
      "single-applicant-b",
      fixedNow
    );
    ...
    if (!resubmitted.ok)
      expect(resubmitted.error.code).toBe("DUPLICATE_ACTION");
  });
  ...
  it("manually rejects one submitted Team Proposal while preserving other proposals", () => {
    ...
    const targetApplication = before?.applications.find(
      (item) => item.teamId === targetTeam?.id
    );
    const otherApplication = before?.applications.find(
      (item) => item.teamId === otherTeam?.id
    );
    expect(targetTeam?.status).toBe(QuestTeamStatus.TEAM_SUBMITTED);
    expect(otherTeam?.status).toBe(QuestTeamStatus.TEAM_SUBMITTED);
    expect(targetApplication?.status).toBe(
      QuestApplicationStatus.APPLICATION_APPLIED
    );
    expect(otherApplication?.status).toBe(
      QuestApplicationStatus.APPLICATION_APPLIED
    );
    if (!targetTeam || !otherTeam || !targetApplication || !otherApplication)
      return;
    ...
    if (rejected.ok) {
      expect(rejected.state.quest.status).toBe(QuestStatus.QUEST_OPEN);
      expect(
        rejected.state.teams.find((team) => team.id === targetTeam.id)?.status
      ).toBe(QuestTeamStatus.TEAM_REJECTED);
      expect(
        rejected.state.teams.find((team) => team.id === otherTeam.id)?.status
      ).toBe(QuestTeamStatus.TEAM_SUBMITTED);
      expect(
        rejected.state.applications.find(
          (item) => item.id === targetApplication.id
        )
      ).toEqual(
        expect.objectContaining({
          status: QuestApplicationStatus.APPLICATION_REJECTED,
      ...
      expect(
        rejected.state.applications.find(
          (item) => item.id === otherApplication.id
        )?.status
      ).toBe(QuestApplicationStatus.APPLICATION_APPLIED);
      expect(rejected.state.assignments).toHaveLength(0);
      expect(rejected.state.conversation.conversationId).toBeNull();
      expect(rejected.state.conversation.canWrite).toBe(false);
    }

    const resubmitted = questFixtureAdapter.submitTeam(
      "team-selection-demo",
      targetTeam.leaderId,
      fixedNow
    );
  ...
  it("submits a non-empty partial Team roster and preserves invitation server states", () => {
    ...
    const invitation = created.inviteWorker(
      "team-forming-demo",
      "demo-worker-4",
      "demo-worker-3",
      fixedNow
    ...
    const accepted = created.acceptInvitation(
      "team-forming-demo",
      pending?.id ?? "",
      "demo-worker-4",
      fixedNow
    ...
    const submitted = created.submitTeam(
      "team-forming-demo",
      "demo-worker-3",
      fixedNow
    );
    ...
    if (submitted.ok) {
      ...
      expect(submitted.state.applications[0]?.teamId).toBe(
        submitted.state.team?.id
      );
    }
  });
  ...
  it("accepts multiple submitted Team Proposals and rejects a Worker joining two Teams", () => {
    const created = createQuestFixtureAdapter({ now: fixedNow });
    const initialTeam = created
      .getState("team-forming-demo", "demo-team-leader", fixedNow)
      ?.teams.find((team) => team.leaderId === "demo-team-leader");
    expect(initialTeam).toBeTruthy();
    if (!initialTeam) return;

    const duplicateLeader = created.createTeam(
      "team-forming-demo",
      "demo-team-leader",
      fixedNow
    );
    expect(duplicateLeader.ok).toBe(false);
    if (!duplicateLeader.ok)
      expect(duplicateLeader.error.code).toBe("DUPLICATE_ACTION");

    const firstInvitation = created.inviteWorker(
      "team-forming-demo",
      "demo-worker-4",
      "demo-team-leader",
      fixedNow
    ...
    const secondTeam = created.createTeam(
      "team-forming-demo",
      "demo-worker-3",
      fixedNow
    );
    ...
    const duplicateMembership = created.inviteWorker(
      "team-forming-demo",
      "demo-worker-4",
      "demo-worker-3",
      fixedNow
    ...
    if (!duplicateMembership.ok)
      expect(duplicateMembership.error.code).toBe("DUPLICATE_ACTION");

    const firstSubmitted = created.submitTeam(
      "team-forming-demo",
      "demo-team-leader",
      fixedNow
    );
    const secondSubmitted = created.submitTeam(
      "team-forming-demo",
      "demo-worker-3",
      fixedNow
    );
    ...
    const hirerState = created.getState(
      "team-forming-demo",
      "demo-hirer",
      fixedNow
    );
    expect(hirerState?.applications).toHaveLength(2);
    expect(hirerState?.applications.every((proposal) => proposal.teamId)).toBe(
      true
    );
    expect(
      hirerState?.teams.filter(
        (team) => team.status === QuestTeamStatus.TEAM_SUBMITTED
      )
    ).toHaveLength(2);

    const selected = created.selectTeam(
      "team-forming-demo",
      initialTeam.id,
      "demo-hirer",
      fixedNow
    ...
    if (selected.ok) {
      expect(
        selected.state.teams.find((team) => team.id === initialTeam.id)?.status
      ).toBe(QuestTeamStatus.TEAM_SELECTED);
      expect(
        selected.state.teams.find(
          (team) => team.id === secondTeam.state.team?.id
        )?.status
      ).toBe(QuestTeamStatus.TEAM_REJECTED);
      expect(
        selected.state.applications.filter(
          (proposal) =>
    ...
    const rejectedTeamView = created.getState(
      "team-forming-demo",
      "demo-worker-3",
      fixedNow
    );
    expect(
      rejectedTeamView?.applications.filter(
        (proposal) =>
  ...
  it("selects a partial submitted Team and settles by actual headcount", () => {
    ...
    const submittedTeam = submitted.state.teams.find(
      (candidate) => candidate.leaderId === "demo-worker-3"
    );
  ...
  it("selects a Candidate and creates one Assignment per selected Team member", () => {
    const created = createQuestFixtureAdapter({ now: fixedNow });
    const teamState = created.getState(
      "play-badminton",
      "student-creator-15",
      fixedNow
    );
    const teamApplication = teamState?.applications[0];
    const selected = created.selectCandidate(
      "play-badminton",
      teamApplication?.id ?? "",
      "student-creator-15",
      fixedNow
  ...
  it("limits selected Team completion to the Team Leader", () => {
    const created = createQuestFixtureAdapter({ now: fixedNow });
    const teamState = created.getState(
      "play-badminton",
      "student-creator-15",
    ...
    const afterStart = new Date("2026-08-20T10:00:00.000Z");
    const leaderState = created.getState(
      "play-badminton",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      afterStart
    );
    const memberState = created.getState(
      "play-badminton",
      "demo-worker-2",
      afterStart
    );
    expect(leaderState?.capabilities.availableActions).toContain(
      "CONFIRM_COMPLETION"
    );
    expect(memberState?.capabilities.availableActions).not.toContain(
      "CONFIRM_COMPLETION"
    );

    const memberAttempt = created.confirmCompletion(
      "play-badminton",
      "demo-worker-2",
      afterStart
    );
    expect(memberAttempt.ok).toBe(false);
    if (!memberAttempt.ok) expect(memberAttempt.error.code).toBe("FORBIDDEN");
  });

  it("applies unanimous edit consent and returns the prior Quest state", () => {
    const created = createQuestFixtureAdapter({ now: fixedNow });
    const requested = created.requestEdit(
      "buy-lunch",
      { description: "Updated lunch instructions." },
      "student-creator-4",
      fixedNow
    );
    expect(requested.ok).toBe(true);
    if (!requested.ok) return;
    expect(requested.state.quest.status).toBe(
      QuestStatus.QUEST_AWAITING_EDIT_CONSENT
    );
    expect(requested.state.editConsent?.status).toBe(
      QuestEditRequestStatus.EDIT_REQUEST_PENDING
    );
    expect(getConsentRemainingMs(requested.state.editConsent, fixedNow)).toBe(
      EDIT_CONSENT_WINDOW_MS
    );
    expect(formatConsentCountdown(requested.state.editConsent, fixedNow)).toBe(
      "05:00"
    );

    const voted = created.voteEditConsent(
      "buy-lunch",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      true,
      fixedNow
    ...
    if (voted.ok) {
      ...
      expect(voted.state.editConsent?.status).toBe(
        QuestEditRequestStatus.EDIT_REQUEST_APPROVED
      );
    }
  });
  it("provides in-progress mocks for every completion mode", () => {
    const created = createQuestFixtureAdapter({ now: fixedNow });
    const scenarios = [
      {
    ...
    scenarios.forEach((scenario) => {
      ...
      expect(state?.quest).toEqual(
        expect.objectContaining({
          candidateMode: scenario.candidateMode,
          participation: scenario.participation,
          proofRequired: scenario.proofRequired,
          status: QuestStatus.QUEST_IN_PROGRESS,
        })
      );
      expect(state?.assignments).toHaveLength(scenario.assignmentCount);
      expect(
        state?.assignments.every((item) => item.status === "ASSIGNMENT_ACTIVE")
      ).toBe(true);
      expect(state?.capabilities.availableActions).toContain(scenario.action);
      expect(
        state?.teams.some(
          (team) => team.status === QuestTeamStatus.TEAM_SELECTED
        )
      ).toBe(scenario.hasSelectedTeam);
    });
  });

  it("waits for every direct-group proof before entering review", () => {
    const source = createQuestFixtureAdapter({ now: fixedNow }).getState(
    ...
    source.assignments.push({
      ...firstAssignment,
      id: "fixture-assignment-direct-group-proof-demo-worker-2",
      workerId: "demo-worker-2",
    });
    const created = createQuestFixtureAdapter({
      now: fixedNow,
      states: [source],
    });

    const first = created.submitProof(
      "direct-group-proof-demo",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      ["fixture://first"],
      "First",
      fixedNow
    ...
    if (first.ok)
      expect(first.state.quest.status).toBe(QuestStatus.QUEST_IN_PROGRESS);
    const second = created.submitProof(
      "direct-group-proof-demo",
      "demo-worker-2",
      ["fixture://second"],
      "Second",
    ...
    if (second.ok)
      expect(second.state.quest.status).toBe(QuestStatus.QUEST_IN_PROGRESS);
  });

  it("supports proof submission and proof-free completion through server state", () => {
    const created = createQuestFixtureAdapter({ now: fixedNow });
    const submitted = created.submitProof(
      "proof-in-progress-demo",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      ["fixture://proof"],
      "Done.",
    ...
    expect(submitted.ok).toBe(true);
    if (submitted.ok)
      expect(submitted.state.quest.status).toBe(QuestStatus.QUEST_IN_PROGRESS);

    const proofFree = created.confirmCompletion(
    ...
    if (proofFree.ok)
      expect(proofFree.state.quest.status).toBe(QuestStatus.QUEST_COMPLETED);
  });

  it("validates proof content, attachment count, and send locking", () => {
    const created = createQuestFixtureAdapter({ now: fixedNow });
    const empty = created.submitProof(
      "proof-in-progress-demo",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      [],
      "   ",
    ...
    if (!empty.ok) expect(empty.error.code).toBe("PROOF_NOT_READY");
    const tooLong = created.submitProof(
      "proof-in-progress-demo",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      [],
      "x".repeat(1001),
    ...
    if (!tooLong.ok) expect(tooLong.error.code).toBe("PROOF_NOT_READY");

    const tooMany = created.submitProof(
      "proof-in-progress-demo",
      DEFAULT_PROTOTYPE_VIEWER_ID,
    ...
    if (!tooMany.ok) expect(tooMany.error.code).toBe("PROOF_NOT_READY");
    ...
    expect(source).toBeTruthy();
    if (!source) return;
    source.quest.id = "proof-lock-demo";
    source.quest.participation = "GROUP";
    source.quest.headcount = 2;
    source.assignments.push({
      ...source.assignments[0],
      id: "fixture-assignment-proof-lock-demo-worker-2",
      workerId: "demo-worker-2",
    });
    const grouped = createQuestFixtureAdapter({
      now: fixedNow,
      states: [source],
    });
    const submitted = grouped.submitProof(
      "proof-lock-demo",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      ["fixture://proof"],
      "",
    ...
    const duplicate = grouped.submitProof(
      "proof-lock-demo",
      DEFAULT_PROTOTYPE_VIEWER_ID,
      ["fixture://replacement"],
      "Replacement",
  ...
  it("moves proof through rejection, rework, review, and terminal read-only chat", () => {
    ...
    const reworked = created.submitRework(
      "move-club-equipment",
      proof?.id ?? "",
      "demo-worker-3",
      ["fixture://new-proof"],
    ...
    const reviewed = created.approveProof(
      "move-club-equipment",
      proof?.id ?? "",
      "student-creator-7",
      fixedNow
    ...
    expect(reviewed.state.conversation.canRead).toBe(true);
    expect(reviewed.state.conversation.canWrite).toBe(false);
    expect(reviewed.state.conversation.readOnlyReason).toBe("TERMINAL");
  });
});
