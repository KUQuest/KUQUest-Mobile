...
describe("Quest Board screen", () => {
  ...
  it("renders Quest cards with decision signals and one detail action", async () => {
    ...
    expect(
      view.getByText("Find a Quest that fits your skills and time.")
    ).toBeTruthy();
    ...
    expect(
      view.getByText(
        "Help carry labelled boxes from the parking area to Dorm 13."
      )
    ).toBeTruthy();
    ...
    expect(view.getAllByText("2 of 2 spots left").length).toBeGreaterThan(0);
    expect(
      view.getAllByText("Single person · Apply for review").length
    ).toBeGreaterThan(0);
    expect(
      view.getAllByText("Team · First-come, first-served").length
    ).toBeGreaterThan(0);
  ...
  it("filters immediately from search and shows a recoverable no-match state", async () => {
    ...
    await fireEvent.changeText(
      view.getByTestId("quest-board-search"),
      "deliver drinks"
    );
  ...
  it("suggests tags from search and commits them with location filters from the bottom sheet", async () => {
    ...
    expect(view.getByTestId("quest-filter-sheet").props.style).toEqual(
    ...
    await waitFor(() =>
      expect(view.getByTestId("quest-filter-tag-cleaning")).toBeTruthy()
    );
  ...
  it("filters by reward bounds, tags, and start-time buckets", async () => {
    ...
    await fireEvent.press(
      view.getByTestId("quest-filter-start-time-afternoon")
    );
    await fireEvent.press(view.getByTestId("apply-quest-filters"));
    ...
    await waitFor(() => {
      ...
      expect(
        view.getByTestId("active-quest-filter-start-time-afternoon")
      ).toBeTruthy();
    });
  });
  ...
  it("preserves Quest Board Search when filters are cleared", async () => {
    ...
    expect(view.getByTestId("quest-board-search").props.value).toBe(
      "deliver drinks"
    );
    expect(view.getByText("Deliver drinks to the library")).toBeTruthy();
    expect(view.queryByText("Help move boxes to the dorm")).toBeNull();
  ...
  it("does not offer an unmatched tag or allow creating one", async () => {
    ...
    await waitFor(() =>
      expect(view.getByText("No matching tags")).toBeTruthy()
    );
  ...
  it("keeps Apply disabled for invalid reward bounds", async () => {
    ...
    expect(
      view.getByTestId("apply-quest-filters").props.accessibilityState
  ...
  it("changes deterministic sort order from the sort sheet", async () => {
    ...
    await waitFor(() =>
      expect(view.getByTestId("quest-sort-reward-highest")).toBeTruthy()
    );
  ...
  it("keeps application outcomes out of Quest cards and keeps the action in Quest Detail", async () => {
    const pending = await render(
      <QuestBoardScreen initialPreviewState="application-pending" />
    );
  ...
  it("forwards application preview scenarios into Quest Detail", async () => {
    ...
    expect(mockRouter.push).toHaveBeenCalledWith({
      pathname: "/quest/[id]",
      params: { id: "move-boxes", preview: "application-pending" },
    });
  });
  ...
  it("exposes the dev-only prototype menu without adding scenarios to Board discovery", async () => {
    ...
    await fireEvent.press(
      view.getByTestId("quest-board-prototype-menu-trigger")
    );
    await fireEvent.press(
      view.getByTestId("quest-board-prototype-menu-persona-demo-worker-2")
    );
    await fireEvent.press(
      view.getByTestId("quest-board-prototype-menu-trigger")
    );
    await fireEvent.press(
      view.getByTestId(
        "quest-board-prototype-menu-scenario-single-candidate-demo"
      )
    );
    ...
    await fireEvent.press(
      view.getByTestId("quest-board-prototype-menu-reset-all")
    );
