...
const mockGetSession = authService.getSession as jest.MockedFunction<
...
describe("QuestDetailScreen loading state", () => {
  ...
  it("renders the canonical loading preview without a local database", async () => {
    const view = await render(
      <QuestDetailScreen previewState="loading" questId="print-documents" />
    );
  ...
  it("keeps the loading footer in the pulsing accessibility group with settled spacing", async () => {
    ...
    const loadingFooter = within(loadingGroup).getByTestId(
      "quest-detail-loading-action-bar"
    );
    expect(loadingFooter).toBeTruthy();

    const settled = await render(
      <QuestDetailScreen studentId="student-demo" questId="print-documents" />
    );
    await waitFor(() =>
      expect(settled.getByTestId("quest-action-bar")).toBeTruthy()
    );
    expect(StyleSheet.flatten(loadingFooter.props.style)?.paddingBottom).toBe(
      StyleSheet.flatten(settled.getByTestId("quest-action-bar").props.style)
        ?.paddingBottom
    );
  });
  ...
  it("does not expose Apply until application state hydration completes outside demo mode", async () => {
    let resolveSession!: (
      value: Awaited<ReturnType<typeof authService.getSession>>
    ) => void;
