    ...
    >
      {onBackPress ? (
        <TouchableOpacity
          ...
          hitSlop={8}
          onPress={onBackPress}
          className={cn(styles.backButton, styles.backButtonPosition)}
          style={{
      ...
      {title ? (
        ...
        <Image
          ...
          style={{
            height: isProfile ? metrics.logoHeight : 32,
            width: isProfile ? metrics.logoWidth : 60,
          }}
        />
      )}
      {rightAction ? (
        <View className={styles.rightAction}>{rightAction}</View>
      ) : null}
    </View>
  );
}
