  ...
  return (
    <SafeAreaView edges={['top', 'left', 'right']} className="flex-1 bg-ku-background">
      <View className="flex-1 justify-start p-[24px] pt-[24px]">
        ...
        <Text className="font-ku-bold text-ku-primary text-ku-title text-left">{messages[titleKey]}</Text>
        <Text className="font-ku-regular text-ku-text-secondary mt-[8px] max-w-[360px] text-ku-body text-left">{messages[descriptionKey]}</Text>
        {actionLabel && onAction ? <Pressable accessibilityRole="button" className="self-start bg-ku-primary rounded-ku-pill mt-[24px] min-h-[44px] justify-center px-[24px]" onPress={onAction}><Text className="text-ku-white font-ku-semibold">{actionLabel}</Text></Pressable> : null}
      </View>
    </SafeAreaView>
  );
}
