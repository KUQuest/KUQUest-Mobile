...
export function FeaturePlaceholder({
  ...
  return (
    ...
    >
      <View className="flex-1 justify-start p-ku-lg pt-ku-lg">
        <View className="mb-ku-20 h-[64px] w-[64px] items-center justify-center rounded-[24px] border border-ku-border-accent bg-ku-surface-accent">
          <Icon color={colors.primary} size={32} strokeWidth={2} />
        </View>
        <Text className="text-left font-ku-bold text-ku-title text-ku-primary">
          {messages[titleKey]}
        </Text>
        <Text className="mt-ku-sm max-w-[360px] text-left font-ku-regular text-ku-body text-ku-text-secondary">
          {messages[descriptionKey]}
        </Text>
        {actionLabel && onAction ? (
          <Pressable
            accessibilityRole="button"
            className="mt-ku-lg min-h-[44px] justify-center self-start rounded-ku-pill bg-ku-primary px-ku-lg"
            onPress={onAction}
          >
