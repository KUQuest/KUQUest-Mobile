...
function printHelp() {
  process.stdout
    .write(`Generate stable Android signing keys and upload them to GitHub Environments.
...
function parseArgs(argv) {
  ...
  if (argv.length === 0 || argv[0].startsWith("-")) {
    throw new CliError(
      2,
      "Missing command: use generate or upload",
...
function validateOptions(options) {
  ...
  if (!options.keystore) {
    throw new CliError(
      2,
      "Missing --keystore",
      "Choose a secure path outside the repository."
    );
  }
  ...
  if (!/^[A-Za-z0-9._-]+$/.test(options.alias)) {
    throw new CliError(
      2,
      "Key alias may contain only letters, digits, dot, underscore, and hyphen."
    );
  }
...
async function credentials(confirmValues) {
  const storePassword =
    process.env.KUQUEST_KEYSTORE_PASSWORD ??
    (await readSecret("Keystore password"));
  const keyPassword =
    process.env.KUQUEST_KEY_PASSWORD ?? (await readSecret("Key password"));
  if (storePassword.length < 6 || keyPassword.length < 6) {
    throw new CliError(
      78,
      "Android keystore passwords must contain at least 6 characters."
    );
  }
...
function inspectKeystore(options, signingCredentials) {
  ...
  if (!sha1 || !sha256) {
    throw new CliError(
      1,
      "keytool did not report SHA-1 and SHA-256 fingerprints."
    );
  }
  return { sha1, sha256 };
}
...
async function generate(options) {
  if (fs.existsSync(options.keystore)) {
    throw new CliError(
      5,
      `Keystore already exists: ${options.keystore}`,
      "Use upload or choose a new path; keys are never overwritten."
    );
  }
  if (options.dryRun) {
    return {
  ...
  fs.mkdirSync(path.dirname(options.keystore), {
    mode: 0o700,
    recursive: true,
  });
  run(
    ...
    [
      ...
      "10000",
      "-dname",
      ENVIRONMENT_CONFIG[options.environment].distinguishedName,
      "-storepass:env",
      "KUQUEST_STORE_PASSWORD",
      "-keypass:env",
      "KUQUEST_KEY_PASSWORD",
    ],
...
async function upload(options) {
  if (!fs.existsSync(options.keystore)) {
    throw new CliError(
      3,
      `Keystore does not exist: ${options.keystore}`,
  ...
  if (!options.dryRun) {
    ...
    run(
      "gh",
      ["api", `repos/${repository}/environments/${options.environment}`],
      {
        failureCode: 3,
    ...
    const secrets = {
      ANDROID_KEYSTORE_BASE64: fs
        .readFileSync(options.keystore)
        .toString("base64"),
...
function writeSuccess(options, data) {
  if (options.json) {
    process.stdout.write(
      `${JSON.stringify({ status: "ok", data, warnings: [] })}\n`
    );
    return;
  }
  if (data.planned) {
    process.stdout.write(
      `Would ${data.planned} ${data.environment} signing key at ${data.keystore}.\n`
    );
    return;
  }
  ...
  if (data.uploaded) {
    process.stdout.write(
      `Secrets uploaded to ${data.repository} Environment '${data.environment}'.\n`
    );
  } else {
    process.stdout.write(
